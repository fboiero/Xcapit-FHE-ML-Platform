...
class TestCompletenessCalculation:
    ...
    def test_completeness_perfect(self, calculator):
        ...
        score = calculator.calculate_completeness(record_count=1000, feature_count=10, null_count=0)
        assert score == 100.0
    ...
    def test_completeness_with_nulls(self, calculator):
        ...
        score = calculator.calculate_completeness(
            record_count=1000, feature_count=10, null_count=1000
        )
        assert score == 90.0
    ...
    def test_completeness_half_nulls(self, calculator):
        ...
        score = calculator.calculate_completeness(
            record_count=100, feature_count=10, null_count=500
        )
        assert score == 50.0
    ...
    def test_completeness_with_per_feature_nulls(self, calculator):
        ...
        null_counts = {"feature_0": 100, "feature_1": 200, "feature_2": 50}
        # Total: 350 nulls, 1000 records * 10 features = 10000 cells
        # 350/10000 = 3.5% null = 96.5% completeness
        score = calculator.calculate_completeness(
            record_count=1000, feature_count=10, null_count=0, null_counts_per_feature=null_counts
        )
        assert score == 96.5
...
class TestConsistencyCalculation:
    ...
    def test_consistency_perfect(self, calculator):
        ...
        score = calculator.calculate_consistency(
            record_count=1000, schema_violations=0, format_violations=0, range_violations=0
        )
        assert score == 100.0
    ...
    def test_consistency_with_violations(self, calculator):
        ...
        score = calculator.calculate_consistency(
            record_count=1000, schema_violations=100, format_violations=100, range_violations=100
        )
        assert score == 90.0
    ...
    def test_consistency_all_violations(self, calculator):
        ...
        score = calculator.calculate_consistency(
            record_count=100, schema_violations=100, format_violations=100, range_violations=100
        )
        assert score == 0.0

    def test_consistency_empty_records(self, calculator):
        """Test consistency with no records."""
        score = calculator.calculate_consistency(record_count=0, schema_violations=0)
        assert score == 0.0

    def test_consistency_negative_violations_capped(self, calculator):
        """Test that negative violations are treated as 0."""
        score = calculator.calculate_consistency(record_count=100, schema_violations=-10)
        assert score == 100.0
...
class TestUniquenessCalculation:
    ...
    def test_uniqueness_perfect(self, calculator):
        ...
        score = calculator.calculate_uniqueness(record_count=1000, duplicate_count=0)
        assert score == 100.0
...
class TestValidityCalculation:
    ...
    def test_validity_perfect(self, calculator):
        ...
        score = calculator.calculate_validity(record_count=1000, invalid_count=0)
        assert score == 100.0
    ...
    def test_validity_with_invalid(self, calculator):
        ...
        score = calculator.calculate_validity(record_count=1000, invalid_count=100)
        assert score == 90.0

    def test_validity_empty_records(self, calculator):
        """Test validity with no records."""
        score = calculator.calculate_validity(record_count=0, invalid_count=0)
        assert score == 0.0
...
class TestFreshnessCalculation:
    ...
    def test_freshness_just_updated(self, calculator):
        ...
        now = datetime.utcnow()
        score = calculator.calculate_freshness(last_updated=now, reference_time=now)
        assert score == 100.0
    ...
    def test_freshness_one_day_old(self, calculator):
        ...
        score = calculator.calculate_freshness(last_updated=one_day_ago, reference_time=now)
    ...
    def test_freshness_half_threshold(self, calculator):
        ...
        score = calculator.calculate_freshness(last_updated=fifteen_days_ago, reference_time=now)
    ...
    def test_freshness_at_threshold(self, calculator):
        ...
        score = calculator.calculate_freshness(last_updated=thirty_days_ago, reference_time=now)
        assert score == 0.0
    ...
    def test_freshness_beyond_threshold(self, calculator):
        ...
        score = calculator.calculate_freshness(last_updated=sixty_days_ago, reference_time=now)
        assert score == 0.0
    ...
    def test_freshness_string_timestamp(self, calculator):
        ...
        now = datetime.utcnow()
        score = calculator.calculate_freshness(last_updated=now.isoformat(), reference_time=now)
        assert score == 100.0
    ...
    def test_freshness_future_timestamp(self, calculator):
        ...
        score = calculator.calculate_freshness(last_updated=future, reference_time=now)
        assert score == 100.0
    ...
    def test_freshness_custom_threshold(self):
        ...
        score = calc.calculate_freshness(last_updated=three_days_ago, reference_time=now)
...
class TestOverallScoreCalculation:
    ...
    def test_overall_perfect_scores(self, calculator):
        ...
        score = calculator.calculate_overall(
            completeness=100.0, consistency=100.0, uniqueness=100.0, validity=100.0, freshness=100.0
        )
        assert score == 100.0
    ...
    def test_overall_all_zero(self, calculator):
        ...
        score = calculator.calculate_overall(
            completeness=0.0, consistency=0.0, uniqueness=0.0, validity=0.0, freshness=0.0
        )
        assert score == 0.0
    ...
    def test_overall_mixed_scores(self, calculator):
        ...
        score = calculator.calculate_overall(
            completeness=100.0, consistency=80.0, uniqueness=90.0, validity=70.0, freshness=60.0
        )
...
class TestAssessFromMetadata:
    ...
    def test_assess_from_metadata_includes_counts(self, calculator):
        ...
        result = calculator.assess_from_metadata(record_count=500, feature_count=15)
    ...
    def test_assess_from_metadata_includes_timestamp(self, calculator):
        ...
        result = calculator.assess_from_metadata(record_count=100, feature_count=5)
    ...
    def test_assess_from_metadata_with_custom_metadata(self, calculator):
        ...
        result = calculator.assess_from_metadata(
            record_count=100, feature_count=5, metadata=custom_meta
        )
...
class TestCalculateQualityFromEncryptedMetadata:
    ...
    def test_basic_encrypted_calculation(self):
        ...
        result = calculate_quality_from_encrypted_metadata(
            encrypted_record_count=1000, encrypted_feature_count=10, stats_summary=stats
        )
    ...
    def test_encrypted_calculation_empty_stats(self):
        ...
        result = calculate_quality_from_encrypted_metadata(
            encrypted_record_count=1000, encrypted_feature_count=10, stats_summary={}
        )
...
class TestEdgeCases:
    ...
    def test_very_large_dataset(self, calculator):
        ...
        score = calculator.calculate_completeness(
            record_count=10_000_000, feature_count=100, null_count=1_000_000
        )
    ...
    def test_single_record(self, calculator):
        ...
        score = calculator.calculate_completeness(record_count=1, feature_count=10, null_count=1)
        assert score == 90.0

    def test_single_feature(self, calculator):
        """Test with single feature."""
        score = calculator.calculate_uniqueness(record_count=100, duplicate_count=10)
        assert score == 90.0

    def test_score_rounding(self, calculator):
        """Test that scores are properly rounded."""
        score = calculator.calculate_completeness(record_count=3, feature_count=10, null_count=1)
