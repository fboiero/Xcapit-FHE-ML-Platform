...
class TrialService(BaseService):
    ...
    def get_usage_summary(self, company) -> ServiceResult:
        ...
        from apps.sandbox.models import DataUpload, Sandbox
    ...
    def check_upload_allowed(self, company, file_size: int) -> ServiceResult:
        ...
        from apps.sandbox.models import DataUpload
    ...
    def request_upgrade(self, company, target_tier: str) -> ServiceResult:
        """
        Create a pending upgrade request. The actual tier change only happens
        after payment confirmation via confirm_upgrade().

        SECURITY: Does NOT directly change the tier — requires payment flow.
        """
        from apps.sandbox.models import Subscription
        ...
        # Create a PENDING subscription instead of directly upgrading
        sub = getattr(company, "subscription", None)
        now = timezone.now()
        if sub:
            if sub.status == Subscription.Status.PENDING:
                return ServiceResult.fail(
                    "Ya tienes una solicitud de upgrade pendiente.",
    ...
    def confirm_upgrade(
        self, company, payment_token: str, *, validated_by_gateway: bool = False,
    ) -> ServiceResult:
        """
        Confirm an upgrade after payment verification.

        In production, payment_token is verified against Stripe/payment gateway.
        The validated_by_gateway flag is only set True by the payment webhook handler.
        """
        from apps.sandbox.models import Subscription
        ...
        if not sub or sub.status != Subscription.Status.PENDING:
            return ServiceResult.fail(
                "No hay solicitud de upgrade pendiente.",
                error_code="no_pending_upgrade",
            )

        if not validated_by_gateway:
            return ServiceResult.fail(
                "El pago no ha sido verificado por el gateway.",
                error_code="payment_not_verified",
            )

        old_tier = company.tier
        target_tier = sub.plan
