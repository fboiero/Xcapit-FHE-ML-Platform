...
class CompetitiveInsightsManager:
    ...
    def compare_to_industry(
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                comparison_id, company_id, industry, len(comparisons), overall_percentile,
                json.dumps(strengths), json.dumps(improvements), json.dumps({"comparisons": comparisons})
            ))
            conn.commit()
    ...
    def get_competitive_position(
        ...
        ranking = "top_decile" if avg_percentile >= 90 else \
                  ...
                  "above_median" if avg_percentile >= 50 else "below_median"
