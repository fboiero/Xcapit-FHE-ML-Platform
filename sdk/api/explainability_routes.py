...
async def list_explanations(
    ...
    if not consortium:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Consortium not found")
...
async def get_explainability_stats(
    ...
    return {"stats": stats, "company_id": company["id"], "privacy_preserved": True}
...
async def delete_explanation(
    ...
    try:
        with manager._get_connection() as conn:
            ...
            cursor.execute("DELETE FROM explanation_results WHERE request_id = ?", (request_id,))

            # Delete request
            cursor.execute("DELETE FROM explanation_requests WHERE id = ?", (request_id,))

            conn.commit()

        return {"message": "Explanation deleted successfully", "request_id": request_id}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
