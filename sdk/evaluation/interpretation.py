...
class SHAPApproximation:
    ...
    def _kernel_shap_weights(
        self, n_features: int, n_coalitions: int
        ...
        for _ in range(n_coalitions):
            ...
            s = np.sum(mask)
            if 0 < s < n_features:
                weight = (n_features - 1) / (np.math.comb(n_features, s) * s * (n_features - s))
            else:
...
class FeatureInteraction:
    ...
    def compute_h_statistic(
        ...
        for i, _val_1 in enumerate(grid_1):
            for j, _val_2 in enumerate(grid_2):
...
def explain_prediction(
    ...
    contributions = sorted(
        zip(feature_names, X_sample[0], shap_values), key=lambda x: abs(x[2]), reverse=True
    )
    ...
    return {
        ...
        "top_contributors": [
            {"feature": f, "value": v, "contribution": c} for f, v, c in contributions[:5]
        ],
    }
